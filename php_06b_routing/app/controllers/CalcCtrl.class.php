	...
	public function validate() {
        ...
        if (! getMessages()->isError()) {
            ...
            if (inRole('user')) {
                ...
                if (floatval($this->form->kwota) > 50000) {
                    getMessages()->addError('Użytkownik może wziąć kredyt tylko do 50000 PLN');
                }
                
                if (floatval($this->form->procent) < 10) {
                    getMessages()->addError('Minimalne oprocentowanie dla użytkownika to 10%');
                }
                
            } else if (inRole('admin')) {
                
                if (floatval($this->form->procent) < 2) {
                    getMessages()->addError('Minimalne oprocentowanie dla administratora to 2%');
                }
            }
        }
